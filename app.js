...
function nearbyQueries(r){
  const base = `${r.resort} ${r.city}`.trim();
  return {
    attractionMap: `${base} 주변 관광지`,
    restaurantMap: `${base} 주변 맛집`,
    cafeMap: `${base} 주변 카페`,
    familyMap: `${base} 아이랑 가볼만한 곳`,
    attractionWeb: `${base} 근처 유명 관광지 추천`,
    restaurantWeb: `${base} 근처 유명 맛집 식당 추천`,
    travelCourse: `${r.city} ${r.resort} 여행 코스 맛집 관광지`
  };
}
...
function row(r){
 ...
 return `<tr><td class="num">${r.no}</td><td class="date"><b>${r.date.slice(5).replace('-','/')}</b> ${dayBadge(r.day)}</td><td>${nightBadge(r.nights)}</td><td class="place"><b>${esc(r.resort)}</b><span class="region-tag">${esc(r.city)}</span></td><td class="room-cell"><div>${esc(cleanRoom(r))}</div>${statusText(r,api)}</td><td class="cap">${capBlock(r,api)}<small>${esc(grade)}</small></td><td class="api-cell">${apiBlock(api)}</td><td class="money">${money(r.price)}</td><td><span class="applicants">${r.applicants}</span></td><td class="actions-cell"><button class="detail" onclick="openModal(${r.no})">▣ 상세</button><button class="detail nearby-btn" onclick="openNearbyModal(${r.no})">📍 주변</button>${imgLink}<a class="home" href="${esc(r.brandHomeUrl || searchUrl(q.officialRoom,'web'))}" target="_blank">공식홈</a></td></tr>`;
}
function openModal(no){
 ...
 $('modalBody').innerHTML=`<h2>${esc(r.resort)}</h2><p class="sub">${esc(r.city)} · ${esc(r.roomType)}</p><div class="grid"><div><label>일정</label><b>${esc(r.date)} (${esc(r.day)}) · ${r.nights}박</b></div><div><label>요금</label><b>${money(r.price)}</b></div><div><label>자동추정 기준/최대</label><b>${r.baseGuests||'?'}인 / ${r.maxGuests||'?'}인</b></div><div><label>인원추가비</label><b>${esc(r.extraGuestFee)}</b></div><div><label>평수/등급</label><b>${esc(r.sizeHint)}</b></div><div><label>매핑 근거</label><b>${esc(r.capacityRule)}</b></div></div>${apiPanel}<div class="actions">${r.brandHomeUrl?`<a href="${esc(r.brandHomeUrl)}" target="_blank">공식 홈페이지</a>`:''}${hasApiImage(api)?`<a href="${esc(api.imageUrl)}" target="_blank">API 객실이미지</a>`:''}<a href="${searchUrl(q.officialRoom,'web')}" target="_blank">공식 객실 페이지 찾기</a><a href="${searchUrl(q.officialImage,'duck')}" target="_blank">공식 홈페이지 이미지</a><a href="${searchUrl(q.capacity,'web')}" target="_blank">정원 확인</a><a href="${searchUrl(q.extraFee,'web')}" target="_blank">인원추가비 확인</a><button onclick="openNearbyModal(${r.no})">주변 관광지·맛집 보기</button></div><p class="hint">API 검증필요·확인필요·미사용권장 항목은 확정 정원으로 사용하지 마세요. 최종 신청 전 공식 홈페이지 확인이 필요합니다.</p>`;
 $('modal').classList.add('open'); document.body.style.overflow='hidden';
}
function openNearbyModal(no){
 const r=ROOMS.find(x=>x.no===no); if(!r) return;
 const q=nearbyQueries(r);
 $('modalBody').innerHTML=`<h2>📍 ${esc(r.resort)} 주변 정보</h2><p class="sub">${esc(r.city)} · 숙소 근방 유명 관광지와 맛집 확인용 검색 링크입니다.</p><div class="grid"><div><label>숙소</label><b>${esc(r.resort)}</b></div><div><label>지역</label><b>${esc(r.city)} / ${esc(r.region)}</b></div><div><label>일정</label><b>${esc(r.date)} (${esc(r.day)}) · ${r.nights}박</b></div><div><label>룸타입</label><b>${esc(cleanRoom(r))}</b></div></div><h3>지도에서 바로 확인</h3><div class="actions"><a href="${searchUrl(q.attractionMap,'map')}" target="_blank">네이버지도 관광지</a><a href="${searchUrl(q.restaurantMap,'map')}" target="_blank">네이버지도 맛집</a><a href="${searchUrl(q.cafeMap,'map')}" target="_blank">네이버지도 카페</a><a href="${searchUrl(q.familyMap,'map')}" target="_blank">아이랑 갈만한 곳</a></div><h3>추천 글 검색</h3><div class="actions"><a href="${searchUrl(q.attractionWeb,'web')}" target="_blank">관광지 추천 검색</a><a href="${searchUrl(q.restaurantWeb,'web')}" target="_blank">맛집·식당 추천 검색</a><a href="${searchUrl(q.travelCourse,'web')}" target="_blank">여행 코스 검색</a></div><p class="hint">검색 결과는 네이버지도/웹 검색 기반 참고용입니다. 영업시간, 휴무일, 예약 가능 여부, 이동시간은 방문 전에 별도로 확인하세요.</p>`;
 $('modal').classList.add('open'); document.body.style.overflow='hidden';
}
