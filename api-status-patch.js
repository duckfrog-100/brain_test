...
function filtered(){
 ...
 return ROOMS.filter(r=>{
  ...
  return (!f.q||txt.includes(f.q)) && (!f.date||r.date===f.date) && (!f.region||r.region===f.region) && (!f.brand||r.brand===f.brand) && (!f.cap||displayMax>=f.cap) && (!f.api||apiStatusForFilterPatched(api)===f.api);
 });
}
function render(){
 const rows=filtered();
 $('count').innerHTML=`<b>${rows.length}</b>건 표시 중`;
 const grouped={};
 rows.forEach(r=>(grouped[r.brand]??=[]).push(r));
...
function resetFilters(){ ['q','region','brand','cap','verified','dateFilter'].forEach(id=>{ if($(id)) $(id).value=''; }); render(); }
['q','dateFilter','region','brand','cap','verified'].forEach(id=>{ const el=$(id); if(el){ el.onchange=render; el.oninput=render; }});
render();