...
function filtered(){
 ...
 return ROOMS.filter(r=>{
  ...
  const txt=[r.no,r.resort,r.roomType,r.city,r.region,r.brand,r.date,r.day,api?.apiRoomName,api?.note,api?.status,api?.usability,apiRuleText(api)].join(' ').toLowerCase();
  return (!q||txt.includes(q)) && (!reg||r.region===reg) && (!br||r.brand===br) && (!cap||displayMax>=cap) && (!vf||apiStatusForFilterPatched(api)===vf);
 });
}
